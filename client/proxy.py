...
from utils import Message, GetMsgException, SendMsgException, CHAT_SERVER, User
...
class ProxyMsg:
    def get_messages() -> list[Message]:
        ...
        """
        url = f"{CHAT_SERVER}/v2/chat/messages"
    ...
    def send_message(msg: Message):
        ...
        send a new message
        """
        url = f"{CHAT_SERVER}/v2/chat/message"
...
class ProxyUser:
    def get_or_create_user(user: User):
        ...
        """
        url = f"{CHAT_SERVER}/v2/user/user"
        log.debug(f"client send POST request with url {url}")
        
        json_compatible_user_data: dict = jsonable_encoder(user)
        
        resp = requests.post(url=url, json=json_compatible_user_data, timeout=60, allow_redirects=False)
        if resp.status_code == 422:
